        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
               ...
               fixed4 col = tex2D(_MainTex, i.uv);
                ...
                float clampedNightLight = smoothstep(1 -_NightLightValue, 1, nightLightSample);    
                float4 coloredNightLightSample = lerp(float4(0,0,0,0), _NightColor, clampedNightLight);

                float3 normal = normalize(i.worldNormal);
                float NdotL = dot(_WorldSpaceLightPos0, normal);
                
                float4 clampedWithLightNightLight = NdotL < 0 ? coloredNightLightSample : float4(0,0,0,0);
